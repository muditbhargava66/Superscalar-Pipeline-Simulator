...
including branch prediction, cache hierarchy, register renaming, power modeling,
and performance counters (v1.2.0).
"""
...
def demonstrate_branch_prediction():
    ...
    for i, actual in enumerate(branch_pattern):
        ...
        print(
            f"   PC: {pc:x}, Predicted: {prediction.taken}, "
        ...
        if cycle >= 2:
            for rob_id in range(min(3, len(instructions))):
                if rob_id not in completed_instructions:
                    if renamer.complete_instruction(rob_id, result=42):
...
def demonstrate_performance_counters():
    """Demonstrate comprehensive performance counters (v1.2.0 feature)."""
    print("\nPerformance Counters (v1.2.0)")
    print("-" * 40)

    counters = PerformanceCounters()

    # Simulate a multi-cycle workload
    workload = [
        (2, 5),  # cycle 1: 2 issued, 5 in flight
        (3, 7),  # cycle 2: 3 issued, 7 in flight
        (1, 6),  # cycle 3: 1 issued, 6 in flight
        (4, 8),  # cycle 4: 4 issued (multi-issue), 8 in flight
        (0, 4),  # cycle 5: 0 issued (stall), 4 in flight
        (2, 3),  # cycle 6: 2 issued, 3 in flight
    ]

    print("Simulating 6-cycle workload:")
    for cycle_idx, (issued, in_flight) in enumerate(workload):
        counters.record_cycle(
            instructions_issued=issued, instructions_in_flight=in_flight
        )
        status = "STALL" if issued == 0 else f"{issued} issued"
        print(f"   Cycle {cycle_idx + 1}: {status}, {in_flight} in-flight")
    ...
    hazard_stats = {
        ...
        "instructions_completed": 25,
    }
    counters.update_from_hazard_controller(hazard_stats)

    # Simulate branch outcomes
    counters.record_branch_outcome(predicted=True, actual=True, penalty_cycles=0)
    counters.record_branch_outcome(predicted=True, actual=False, penalty_cycles=15)
    counters.record_branch_outcome(predicted=False, actual=False, penalty_cycles=0)
    counters.record_branch_outcome(predicted=True, actual=True, penalty_cycles=0)
    ...
    print("\nPerformance Report:")
    cycle_info = report["cycle_counters"]
    print(f"   Total Cycles: {cycle_info['total_cycles']}")
    print(f"   Busy Cycles: {cycle_info['busy_cycles']}")
    print(f"   Pipeline Utilization: {cycle_info['pipeline_utilization_pct']:.1f}%")
    ...
    print(
        f"   Structural: {hazards['structural_hazards']}, Control: {hazards['control_hazards']}"
    )
    print(f"   Total Hazards: {hazards['total_hazards']}")

    print("\nBranch Prediction:")
    branch = report["branch_counters"]
    print(f"   Predictions: {branch['total_predictions']}")
    ...
    print(f"   Multi-Issue Cycles: {ilp['multi_issue_cycles']}")
...
def demonstrate_power_modeling():
    ...
    for cycle, (instr_type, unit) in enumerate(
        zip(instruction_types, units, strict=False)
    ):
        # Create a dummy instruction for power modeling
        dummy_instruction = Instruction(
            0x1000 + cycle * 4, "ADD", ["$t0", "$t1", "$t2"]
        )
...
def main():
    ...
    try:
        ...
        print("   \u2022 Pipeline hazard analysis with performance counters")
