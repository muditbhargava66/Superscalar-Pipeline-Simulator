...
def demonstrate_default_configuration():
    ...
    print(f"   L1 Instruction Size: {getattr(memory_config.instruction_cache, 'size', 32768) // 1024} KB")
    print(f"   L1 Data Size: {getattr(memory_config.data_cache, 'size', 32768) // 1024} KB")
    ...
    print("\nExecution Units:")
    pipeline_config = default_config.pipeline
    print(f"   ALU Count: {getattr(pipeline_config.execute_units.get('ALU', {}), 'count', 2)}")
    print(f"   FPU Count: {getattr(pipeline_config.execute_units.get('FPU', {}), 'count', 1)}")
    print(f"   LSU Count: {getattr(pipeline_config.execute_units.get('LSU', {}), 'count', 1)}")
...
def demonstrate_custom_configuration():
    ...
    print(f"   L1 Cache: {custom_config['cache']['l1d_size']} KB")
    print(f"   Branch Predictor: {custom_config['branch_predictor']['type']}")
    print(f"   Physical Registers: {custom_config['register_file']['physical_registers']}")
    ...
    try:
        ...
        print(f"   Pipeline Width: {validated_config.pipeline.fetch_width}")
...
def demonstrate_environment_overrides():
    ...
    print(f"   L1D Cache Size: {getattr(config.memory.data_cache, 'size', 32768) // 1024} KB")
    print(f"   Branch Predictor: {getattr(config.branch_predictor, 'type', 'default')}")
    
    # Clean up environment variables
    for key in ['SIMULATOR_PIPELINE_WIDTH', 'SIMULATOR_CACHE_L1D_SIZE', 'SIMULATOR_BRANCH_PREDICTOR_TYPE']:
...
def demonstrate_configuration_profiles():
    ...
    for profile_name, profile_config in profiles.items():
        ...
        print(f"   Pipeline: {pipeline.get('width', 'default')} wide, {pipeline.get('depth', 'default')} deep")
        print(f"   Cache: L1D {cache.get('l1d_size', 'default')} KB, L2 {cache.get('l2_size', 'default')} KB")
        print(f"   Units: {units.get('alu_count', 'default')} ALU, {units.get('fpu_count', 'default')} FPU")
...
def demonstrate_configuration_validation():
    ...
    for invalid_config in invalid_configs:
        ...
        try:
            ...
            print(f"{invalid_config['description']}: CORRECTLY REJECTED - {str(e)[:50]}...")
...
def main():
    ...
    try:
        ...
        print("   • Predefined configuration profiles")
