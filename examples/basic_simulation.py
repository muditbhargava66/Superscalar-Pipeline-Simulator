...
Includes v1.2.0 performance counters and enhanced statistics.
"""
...
def run_basic_simulation():
    ...
    print(
        f"  Cache Size: {getattr(config.memory.data_cache, 'size', 32768) // 1024} KB"
    )
    ...
    # Demonstrate v1.2.0 performance counters
    print("\nPerformance Counters (v1.2.0):")
    perf_counters = PerformanceCounters()

    # Simulate a few cycles
    for i in range(min(5, len(instructions))):
        perf_counters.record_cycle(instructions_issued=1, instructions_in_flight=i)

    report = perf_counters.get_detailed_report()
    print(f"   Total Cycles: {report['cycle_counters']['total_cycles']}")
    print(f"   Busy Cycles: {report['cycle_counters']['busy_cycles']}")
    print(
        f"   Pipeline Utilization: {report['cycle_counters']['pipeline_utilization_pct']:.1f}%"
    )
    print(f"   Multi-Issue Rate: {report['ilp_counters']['multi_issue_rate_pct']:.1f}%")
