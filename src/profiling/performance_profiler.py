...
class PerformanceProfiler:
    ...
    def __init__(self, enable_detailed_profiling: bool = True):
        ...
        self._initial_cpu_times: psutil._common.pcputimes | None = None  # type: ignore[name-defined]
        self._initial_memory_info: psutil._common.pmem | None = None  # type: ignore[name-defined]
    ...
    def _calculate_cpu_usage(
        self,
        initial: psutil._common.pcputimes,  # type: ignore[name-defined]
        final: psutil._common.pcputimes,  # type: ignore[name-defined]
        execution_time: float,
    ) -> dict[str, float]:
    ...
    def _extract_top_functions(
        self, stats: pstats.Stats, sort_key: str, limit: int
    ) -> list[dict[str, Any]]:
        """Extract top functions by specified metric."""
        stats.sort_stats(sort_key)

        top_functions = []
        for func_info, (cc, nc, tt, ct, callers) in list(stats.stats.items())[:limit]:  # type: ignore[attr-defined]
