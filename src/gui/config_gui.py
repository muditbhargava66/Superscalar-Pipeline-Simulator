...
class ConfigurationGUI:
    ...
    def _create_widgets(self):
        ...
        ttk.Button(
            button_frame, text="Reset to Defaults", command=self._reset_config
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            button_frame, text="Run Simulator", command=self._run_simulator
    ...
    def _create_pipeline_tab(self, parent):
        ...
        # Pipeline parameters
        ttk.Label(
            parent, text="Pipeline Configuration", font=("Arial", 12, "bold")
        ).pack(pady=10)
        ...
        self.fetch_width_var = tk.StringVar(
            value=str(self.config["pipeline"]["fetch_width"])
        )
        ttk.Entry(frame, textvariable=self.fetch_width_var, width=10).grid(
            row=0, column=1, padx=10
        )

        # Issue width
        ttk.Label(frame, text="Issue Width:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.issue_width_var = tk.StringVar(
            value=str(self.config["pipeline"]["issue_width"])
        )
        ttk.Entry(frame, textvariable=self.issue_width_var, width=10).grid(
            row=1, column=1, padx=10
        )

        # Execution units
        ttk.Label(parent, text="Execution Units", font=("Arial", 10, "bold")).pack(
            pady=(20, 5)
        )
        ...
        # ALU count
        ttk.Label(units_frame, text="ALU Count:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.alu_count_var = tk.StringVar(
            value=str(self.config["pipeline"]["execute_units"]["ALU"]["count"])
        )
        ttk.Entry(units_frame, textvariable=self.alu_count_var, width=10).grid(
            row=0, column=1, padx=10
        )

        # FPU count
        ttk.Label(units_frame, text="FPU Count:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.fpu_count_var = tk.StringVar(
            value=str(self.config["pipeline"]["execute_units"]["FPU"]["count"])
        )
        ttk.Entry(units_frame, textvariable=self.fpu_count_var, width=10).grid(
            row=1, column=1, padx=10
        )

        # LSU count
        ttk.Label(units_frame, text="LSU Count:").grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        self.lsu_count_var = tk.StringVar(
            value=str(self.config["pipeline"]["execute_units"]["LSU"]["count"])
        )
        ttk.Entry(units_frame, textvariable=self.lsu_count_var, width=10).grid(
    ...
    def _create_branch_predictor_tab(self, parent):
        ...
        ttk.Label(
            parent, text="Branch Predictor Configuration", font=("Arial", 12, "bold")
        ).pack(pady=10)
        ...
        # Predictor type
        ttk.Label(frame, text="Predictor Type:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        ...
        # Number of entries
        ttk.Label(frame, text="Number of Entries:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
        self.bp_entries_var = tk.StringVar(
            value=str(self.config["branch_predictor"]["num_entries"])
        )
        ttk.Entry(frame, textvariable=self.bp_entries_var, width=10).grid(
            row=1, column=1, padx=10
        )

        # History length
        ttk.Label(frame, text="History Length:").grid(
            row=2, column=0, sticky=tk.W, pady=5
        )
        self.bp_history_var = tk.StringVar(
            value=str(self.config["branch_predictor"]["history_length"])
        )
        ttk.Entry(frame, textvariable=self.bp_history_var, width=10).grid(
            row=2, column=1, padx=10
        )

    def _create_cache_tab(self, parent):
        """Create cache configuration tab."""
        ttk.Label(parent, text="Cache Configuration", font=("Arial", 12, "bold")).pack(
            pady=10
        )

        # Instruction cache
        ttk.Label(parent, text="Instruction Cache", font=("Arial", 10, "bold")).pack(
            pady=(10, 5)
        )
        ...
        ttk.Label(icache_frame, text="Size (bytes):").grid(
        ...
        ttk.Label(icache_frame, text="Block Size:").grid(
        ...
        # Data cache
        ttk.Label(parent, text="Data Cache", font=("Arial", 10, "bold")).pack(
            pady=(20, 5)
        )
        ...
        ttk.Label(dcache_frame, text="Size (bytes):").grid(
            row=0, column=0, sticky=tk.W, pady=2
        )
        ...
        ttk.Label(dcache_frame, text="Block Size:").grid(
            row=1, column=0, sticky=tk.W, pady=2
        )
        ...
        # Memory size
        ttk.Label(parent, text="Main Memory", font=("Arial", 10, "bold")).pack(
            pady=(20, 5)
        )
        ...
        ttk.Label(mem_frame, text="Memory Size (bytes):").grid(
            row=0, column=0, sticky=tk.W, pady=2
        )
    ...
    def _create_simulation_tab(self, parent):
        ...
        ttk.Label(
            parent, text="Simulation Configuration", font=("Arial", 12, "bold")
        ).pack(pady=10)
        ...
        self.max_cycles_var = tk.StringVar(
            value=str(self.config["simulation"]["max_cycles"])
        )
        ttk.Entry(frame, textvariable=self.max_cycles_var, width=10).grid(
            row=0, column=1, padx=10
        )
        ...
        self.output_file_var = tk.StringVar(
            value=self.config["simulation"]["output_file"]
        )
        ttk.Entry(frame, textvariable=self.output_file_var, width=30).grid(
            row=1, column=1, padx=10
        )

        # Checkboxes
        self.enable_viz_var = tk.BooleanVar(
            value=self.config["simulation"]["enable_visualization"]
        )
        ttk.Checkbutton(
            frame, text="Enable Visualization", variable=self.enable_viz_var
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=5)

        self.enable_prof_var = tk.BooleanVar(
            value=self.config["simulation"]["enable_profiling"]
        )
        ttk.Checkbutton(
            frame, text="Enable Profiling", variable=self.enable_prof_var
        ).grid(row=3, column=0, columnspan=2, sticky=tk.W, pady=5)

        # Debug options
        ttk.Label(parent, text="Debug Options", font=("Arial", 10, "bold")).pack(
            pady=(20, 5)
        )
        ...
        ttk.Checkbutton(
            debug_frame, text="Enable Debug", variable=self.debug_enabled_var
        ).grid(row=0, column=0, sticky=tk.W, pady=5)

        ttk.Label(debug_frame, text="Log Level:").grid(
            row=1, column=0, sticky=tk.W, pady=5
        )
    ...
    def _update_config_from_gui(self):
        ...
        try:
            ...
            self.config["branch_predictor"]["num_entries"] = int(
                self.bp_entries_var.get()
            )
            self.config["branch_predictor"]["history_length"] = int(
            ...
            self.config["simulation"]["enable_visualization"] = (
                self.enable_viz_var.get()
            )
