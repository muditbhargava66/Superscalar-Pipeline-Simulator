...
class ExecutionUnitConfig(BaseModel):
    ...
    latency: int = Field(
        default=1, ge=1, le=10, description="Execution latency in cycles"
    )
...
class PipelineConfig(BaseModel):
    ...
    num_stages: int = Field(
        default=6, ge=3, le=10, description="Number of pipeline stages"
    )

    fetch_width: int = Field(
        default=4, ge=1, le=8, description="Instructions fetched per cycle"
    )

    issue_width: int = Field(
        default=4, ge=1, le=8, description="Instructions issued per cycle"
    )
...
class BranchPredictorConfig(BaseModel):
    ...
    type: BranchPredictorType = Field(
        default=BranchPredictorType.GSHARE, description="Branch predictor algorithm"
    )

    num_entries: int = Field(
        default=1024, ge=64, le=65536, description="Number of predictor entries"
    )

    history_length: int = Field(
        default=8, ge=1, le=16, description="Global history length (for gshare)"
    )
...
class CacheConfig(BaseModel):
    ...
    size: int = Field(
        default=32768, ge=1024, le=1048576, description="Cache size in bytes"
    )

    block_size: int = Field(
        default=64, ge=16, le=256, description="Cache block size in bytes"
    )

    associativity: int = Field(
        default=4, ge=1, le=16, description="Cache associativity (1 = direct mapped)"
    )
...
class MemoryConfig(BaseModel):
    ...
    memory_size: int = Field(
        default=1048576, ge=65536, le=134217728, description="Main memory size in bytes"
    )
...
class SimulationConfig(BaseModel):
    ...
    max_cycles: int = Field(
        default=10000, ge=100, le=10000000, description="Maximum simulation cycles"
    )

    output_file: str | None = Field(
        default="simulation_results.txt", description="Output file for results"
    )

    enable_visualization: bool = Field(
        default=False, description="Enable pipeline visualization"
    )

    enable_profiling: bool = Field(
        default=True, description="Enable performance profiling"
    )

    enable_statistics: bool = Field(
        default=True, description="Enable detailed statistics collection"
    )
...
class DebugConfig(BaseModel):
    ...
    trace_instructions: bool = Field(
        default=False, description="Enable instruction tracing"
    )

    trace_pipeline: bool = Field(
        default=False, description="Enable pipeline stage tracing"
    )
...
class SimulatorConfig(BaseModel):
    ...
    simulation: SimulationConfig = Field(
        default_factory=SimulationConfig, description="Simulation configuration"
    )

    debug: DebugConfig = Field(
        default_factory=DebugConfig, description="Debug configuration"
    )
    ...
    def validate_config_consistency(self) -> SimulatorConfig:
        ...
        total_cache_size = (
            self.memory.instruction_cache.size + self.memory.data_cache.size
        )
