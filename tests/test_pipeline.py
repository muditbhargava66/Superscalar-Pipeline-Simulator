...
and their integration, including fetch, decode, issue, execute,
...
class TestPipelineStages(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.instruction_cache = InstructionCache(
            cache_size=1024, block_size=64, memory=self.memory, fetch_bandwidth=4
        )
        self.data_cache = DataCache(cache_size=1024, block_size=64)
        ...
        self.fetch_stage = FetchStage(
            self.instruction_cache, self.branch_predictor, self.memory
        )
        ...
        self.memory_access_stage = MemoryAccessStage(self.data_cache, self.memory)
