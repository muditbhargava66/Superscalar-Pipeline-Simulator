...
including instructions, reservation stations, functional units, cache systems,
register files, scoreboards, data forwarding, pipeline stages, and performance
profiling.
"""
...
class TestReservationStation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.instruction = Instruction(
            address=0, opcode="ADD", operands=["$t0", "$t1", "$t2"]
        )
    ...
    def test_operand_resolution(self):
        ...
        executed_instructions = [
            (Instruction(address=4, opcode="ADDI", operands=["$t1", "$zero", "5"]), 5)
        ]
